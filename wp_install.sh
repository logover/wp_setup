...
# ==============================================================================
# 全自动 WordPress LEMP + 灵活 SSL 安装脚本
# Fully Automated WordPress on LEMP + Flexible SSL Installer (Multi-Distro)
#
# Author: logover
# Version: 1.0
...
install_wordpress() {
    ...
    # ... (Database logic is cross-platform and remains unchanged)
...
MYSQL_SCRIPT
    ...
    # ... (SSL logic is largely cross-platform and remains unchanged)
    ...
    # ... (SSL detection logic remains the same)
...
EOF
    ...
    if [ "$site_protocol" = "https" ]; then
        # ... (acme.sh cert installation logic is unchanged)
        if [[ "$opt" == *"acme.sh"* ]]; then
        ...
        # Use OS-specific PHP FPM socket path
        cat > "$NGINX_CONF_PATH" <<-EOF
		server {
		    listen 80; listen [::]:80;
		    server_name $domain;
		...
		server {
		    ...
		    location ~ \.php\$ {
		        include snippets/fastcgi-php.conf; # This might need adjustment for RHEL
...
uninstall_wordpress() {
    ...
    local DOMAIN="" WP_PATH="" NGINX_CONF="" DB_NAME="" DB_USER="" ACME_ECC="" ACME_HOME_DIR=""
    ...
    if [ -n "$WP_PATH" ]; then
        # ... (Deletion logic is mostly cross-platform and unchanged)
    ...
    if [[ "$purge_packages" == "y" || "$purge_packages" == "Y" ]]; then
        info "正在清除软件包..."
        # Use OS-specific purge command and package list
        $PKG_CMD_PURGE $(echo "$REQUIRED_PACKAGES" | sed 's/php-mysqlnd/php-*/' | sed 's/php-pecl-imagick//' | sed 's/php-fpm/php*/')
